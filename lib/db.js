...
const mongoUrl = MONGO_URL || 'mongodb://127.0.0.1:27017/test';

function getClient() {
  return new MongoClient(mongoUrl);
}

function globalDB(client) {
  return client.db(GLOBAL_NAMESPACE);
}
...
class Instance {
  static type() { return INSTANCE_API_VERSION; }
  ...
  static async findOne(db, entityId, instanceId) {
    const result = await db.collection(entityId).findOne({ id: instanceId });

    return result ? removePrivateFieldsFromObject(result) : result;
  }
  ...
  static async create(db, entityId, instanceId, body) {
    ...
    return removePrivateFieldsFromObject(obj);
  }
  ...
  static async delete(db, entityId, instanceId) {
    ...
    return result && result.value ? removePrivateFieldsFromObject(result.value) : null;
  }
...
class Entity extends SystemInstance {
  ...
  static async create(db, entityId, body) {
    const entity = await super.create(db, entityId, body);
    if (entity) {
